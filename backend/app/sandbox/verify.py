...
def _find_npm_command() -> Optional[str]:
    for name in ("npm.cmd", "npm.exe", "npm"):
        ...
        if p:
            return p
    try:
        out = subprocess.check_output(["where.exe", "npm"], text=True, stderr=subprocess.STDOUT)
...
def verify_repo(repo_dir: Path) -> VerifyResponse:
    ...
    if (repo_dir / "package.json").exists():
        ...
        try:
            ...
            checks["package_json"] = {"ok": False, "reason": f"Failed to read package.json: {e}"}
            return VerifyResponse(ok=False, checks=checks)
    ...
    r = run_cmd(["python", "-c", "print('verify: no tests detected')"], cwd=repo_dir, timeout_s=30)
    checks["fallback"] = r
    return VerifyResponse(ok=True, checks=checks)